...
CREATE POLICY "Users update own offers" ON public.offers
  FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Update own messages" ON public.trade_messages;
CREATE POLICY "Update own messages" ON public.trade_messages
  FOR UPDATE USING (auth.uid() = sender_id) WITH CHECK (auth.uid() = sender_id);

DROP POLICY IF EXISTS "Participants update trade" ON public.trades;
CREATE POLICY "Participants update trade" ON public.trades
  ...
  WITH CHECK (auth.uid() = buyer_id OR auth.uid() = seller_id OR has_role(auth.uid(), 'admin'::app_role));
